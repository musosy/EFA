...
const AuthService = {
    register: async (userDetails: any): Promise<any> => {
        ...
        const userAlreadyExists = (await AuthRepository.getOneByUsername(userDetails.username)).isOk();
        if (userAlreadyExists) return { status: 200, message: 'Username already taken' };
    ...
    securedRouteExemple: () => {
        return {
            status: 200,
            message: 'If you read this message it means you have correctly set your authorization token',
        };
    }
};
